...
`stm` is a state manager for preact. It's main concept is borrowed from ELM Architecture, although it differs in many edge cases. The goal of the `stm` is to create a **web component** which could be plugged into any other framework.
...
In the previous example we created a component that does nothing. To add some interactivity we need to understand the data flow of a stm component:

![STM data flow](/stm-diagram.svg)

There are two most important concepts in `stm`:
...
function view(state: State) {
    ...
    if (state.display === 'question') {
        return <div>
            <input type="text" oninput={(event: Event) => ({ type: 'input', name: (event.target as HTMLInputElement).value })} />
...
3. when user interacts with the view `msg` is returned (see: `<button onclick={{ type: 'introduce' }}>Introduce yourself</button>` i.e.). The event handler can also be a function like in here: `<input type="text" oninput={(event: Event) => ({ type: 'input', name: (event.target as HTMLInputElement).value })} />` - this form can be used when we need an `event` object).
...
Every view is rendered based on state, this means that having a state we can recreate how the app looked like in any moment. Quite often when debugging a `stm` component you will want to know what was the state before certain msg and what was the state after a msg. To show this info please enable `debug` in the `stm.component` arguments:
...
2. the `propTypes` take attribute name and attribute type. The type of attribute is a value constructor (Boolean, Number, String, Object, Array)
...
3. the `cmp` attribute of `willMount` and `willUnmount` is a preact component object. You cannot do much with it (and you shouldn't) however it serves very wall as a holder for storing handlers that should be later removed
