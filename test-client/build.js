(() => {
  ...
  var SwDatetimeInput = class extends _ {
    render({ name, required, placeholder, disabled, value, date, time, step }) {
      ...
      const that = this;
      return /* @__PURE__ */ v(d, null, /* @__PURE__ */ v("style", null, sw_datetime_input_style_default), /* @__PURE__ */ v("div", {
        className: "input-wrapper",
        ref: (node) => this.ref = node,
        customProp: true
        ...
        "aria-labelledby": placeholder,
        onInput: (e2) => {
          const newVal = datetimeToInteger(e2.target.value, time && this.timeVal);
          console.log({ name, newVal, "->": new Date(newVal) });
          if (!isNaN(newVal)) {
            this.dateVal = e2.target.value;
            console.log("I GIT!");
            this.value = newVal;
        ...
        "aria-labelledby": placeholder,
        onInput: (e2) => {
          const newVal = date ? datetimeToInteger(this.dateVal, time && e2.target.value) : timeToInteger(e2.target.value);
          console.log({ name, newVal });
          if (!isNaN(newVal)) {
            this.timeVal = e2.target.value;
            console.log("I GIT!");
            this.value = newVal;
            return this.ref.getRootNode().host.dispatchEvent(new CustomEvent("changeEvent", {
              detail: { name, value: this.value },
